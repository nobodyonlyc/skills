...
## Hard gates
- **Autonomy mode**: honor the run's mode (`gated` default / `auto`) per [autonomy-mode](../../resources/autonomy-mode.md). In `auto`, ask-user gates below become logged decisions — except the always-stop list, which always halts.
- **Durable task state**: maintain `.harness/tasks/<id>.md` per the [task-state convention](../../resources/task-state-convention.md) — created at start, its **granular DoD checklist updated and committed at every phase boundary AND before marking `passing`**, with acceptance criteria each ticked and mapped to a test. This holds **in `auto` mode too** — auto suppresses ask-user gates, never state-keeping. A task file left at an early step while the feature is `passing` is a process failure. `evidence.md` must be non-empty before verify (hook-enforced, see [task-state convention](../../resources/task-state-convention.md)).
