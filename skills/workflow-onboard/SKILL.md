...
- **Durable artifacts**: the survey report and `SYSTEM_MAP.md` go to committed locations (`docs/`), not transient `.harness/reports/` (see the task-state and durable-vs-transient rules in `AGENTS.md` and [task-state-convention](../../resources/task-state-convention.md)).
