...
6. **Verifications**: A list of executable shell commands that **actually exercise the behavior and assert on it** (e.g., `cargo test auth::`, `pytest tests/test_auth.py`, `playwright test checkout`). A bare existence check like `[ -f src/auth.rs ]` is **NOT** acceptance — it proves a file exists, not that it works. Existence checks may only *accompany* real tests, never replace them. Use the **project's real runner syntax** and make sure the command **selects the intended tests and is non-vacuous** — e.g. with jest use `jest <path>` or `-t <name>`, **not** `--grep` (jest treats `--grep` as a path regex, so it can silently run the wrong set or zero tests and still pass — a false green).
