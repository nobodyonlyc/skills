...
- **Empty-request guard (check before everything).** If the user's message is not an actionable request — `.`, `ok`, `hi`, `continue`, a greeting, or blank — do NOT classify or route, and do NOT launch any workflow just because the repo is in bootstrap/empty state. Report current project state (`./harness status` / `./harness resume`, else `claude-progress.md`) and ask the user what they want to do (interactive choice prompt). Project state is not a request.
- **Route first.** *Once there is a real request,* classify it (new project / add-feature / execute-US / bugfix) before writing any code. New project → `/harness-bootstrap`.
...
- **WIP = 1 + auto-advance.** One US at a time. Read `.harness/context.json`: `auto_advance` → ON for `user_role == "Non-Technical"`, **ASK for `Developer`** (default). ON = after a US passes, immediately start the next-highest-priority one without waiting (chain to exhaustion). ASK = don't go idle; show an interactive choice **[Run next US]** / **[Run several / all remaining]** / **[Stop here]**. Hard stops always return control: backlog exhausted, US `blocked`, verify failing, or deploy/destructive action needing sign-off.
<!-- /HARNESS-ANTIGRAVITY-ENFORCEMENT -->
